@echo off
echo Setting up and starting the JWT-Protected Todo API backend server...
echo.
...
REM Start the server with explicit parameters
echo.
...
echo - http://127.0.0.1:8001/health (should return {\"status\": \"healthy\"})
echo - http://127.0.0.1:8001/docs (API documentation)
echo.
